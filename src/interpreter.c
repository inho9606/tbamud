...
cpp_extern const struct command_info cmd_info[] = {
  ...
  { "¾ß±¸"     , "¾ß"       , POS_STANDING , do_baseball , 0, 0 },
  { "Åõ±¸"     , "Åõ"       , POS_STANDING , do_pitch , 0, 0 },
  { "Å¸°Ý"     , "Å¸"       , POS_STANDING , do_bat , 0, 0 },
  { "¼Û±¸"     , "¼Û"       , POS_STANDING , do_pass , 0, 0 },
  ...
  { "¿Ã·Á"    , "¿Ã"     , POS_DEAD , do_stat_plus , 0, 0 },
  ...
  { "ºÒ·¯" , "transfer", POS_SLEEPING, do_trans    , LVL_GOD, 0 },
  ...
  { "¹øÈ£Á¶È¸"     , "vnum"    , POS_DEAD    , do_vnum     , LVL_IMMORT, 0 }, // vnum
  ...
  { "Á¸Ã»¼Ò"   , "zpurge"  , POS_DEAD    , do_zpurge   , LVL_BUILDER, 0 }, // zpurge
...
/*	  	  if (_parse_name(arg, tmp_name)) {
      ...
      if ((player_i = load_char(tmp_name, d->character)) > -1) {
        ...
        if (PLR_FLAGGED(d->character, PLR_DELETED)) {
          ...
          GET_PFILEPOS(d->character) = player_i;
          write_to_output(d, "%s·Î »õ·Î¿î Ä³¸¯ÅÍ¸¦ »ý¼ºÇÏ½Ã°Ú½À´Ï±î? (\t(³×\t)/\t(¾Æ´Ï¿ä\t))?\r\n", tmp_name);
        ...
        if (!valid_name(tmp_name)) {
          write_to_output(d, "»ç¿ëÇÒ ¼ö ¾ø´Â ¾ÆÀÌµðÀÔ´Ï´Ù, ´Ù¸¥ ¾ÆÀÌµð¸¦ ÀÔ·ÂÇØÁÖ¼¼¿ä\r\n¾ÆÀÌµð: ");
          return;
        }
        CREATE(d->character->player.name, char, strlen(tmp_name) + 1);
        strcpy(d->character->player.name, CAP(tmp_name));	/* strcpy: OK (size checked above) */
  ...
  case CON_NAME_CNFRM:		/* wait for conf. of new name    */
	...
	write_to_output(d, "ÇöÀç Á¢¼ÓÁßÀÎ °÷¿¡¼­´Â Ä³¸¯ÅÍ »ý¼ºÀÌ ºÒ°¡´ÉÇÕ´Ï´Ù.\r\n");
	STATE(d) = CON_CLOSE;
	return;
      }
      if (circle_restrict) {
	write_to_output(d, "ÁË¼ÛÇÕ´Ï´Ù, ÇöÀç´Â °¡ÀÔÇÒ ¼ö ¾ø½À´Ï´Ù.\r\n");
	...
	return;
      ...
      write_to_output(d, "È¸¿ø°¡ÀÔÀ» ½ÃÀÛÇÕ´Ï´Ù.\r\n%s´ÔÀÇ ÆÐ½º¿öµå¸¦ ÀÔ·ÂÇØÁÖ¼¼¿ä: ", GET_PC_NAME(d->character));
      echo_off(d);
      STATE(d) = CON_NEWPASSWD;
    } else if (!strcmp(arg, "¾Æ´Ï¿ä")) {
      write_to_output(d, "¾î¶² ÀÌ¸§À» »ç¿ëÇÏ½Ã°Ú½À´Ï±î?\r\n");
      free(d->character->player.name);
      d->character->player.name = NULL;
      STATE(d) = CON_GET_NAME;
    } else
      write_to_output(d, "³× ¶Ç´Â ¾Æ´Ï¿ä·Î ´äÇØÁÖ¼¼¿ä:\r\n");
    break;
  ...
  case CON_PASSWORD:		/* get pwd for known player      */
	...
	if (++(d->bad_pws) >= CONFIG_MAX_BAD_PWS) {	/* 3 strikes and you're out. */
	  write_to_output(d, "ÆÐ½º¿öµå ÀÔ·Â Á¦ÇÑÀ» ÃÊ°úÇÏ¿´½À´Ï´Ù. Á¢¼Ó ÇØÁ¦!\r\n");
	  STATE(d) = CON_CLOSE;
	} else {
	  write_to_output(d, "ÆÐ½º¿öµå°¡ ¸ÂÁö ¾Ê½À´Ï´Ù.\r\nÆÐ½º¿öµå: ");
	  echo_off(d);
	}
	return;
      }
	...
	write_to_output(d, "ÁË¼ÛÇÕ´Ï´Ù, ÇöÀç Á¢¼ÓÁßÀÎ È¯°æ¿¡¼­´Â ·Î±×ÀÎÇÒ ¼ö ¾ø½À´Ï´Ù!\r\n");
	STATE(d) = CON_CLOSE;
	mudlog(NRM, LVL_GOD, TRUE, "Connection attempt for %s denied from %s", GET_NAME(d->character), d->host);
	return;
      }
      if (GET_LEVEL(d->character) < circle_restrict) {
	write_to_output(d, "Áö±ÝÀº Á¢¼ÓÇÒ ¼ö ¾ø½À´Ï´Ù.. ³ªÁß¿¡ ´Ù½Ã Á¢¼ÓÇØ ÁÖ½Ê½Ã¿À.\r\n");
	STATE(d) = CON_CLOSE;
	...
	write_to_output(d, "%s", motd);
      ...
      if (load_result) {
        write_to_output(d, "\r\n\r\n\007\007\007"
		"%s¸¶Áö¸· Á¢¼ÓÀ¸·ÎºÎÅÍ %d¹øÀÇ Àß¸øµÈ Á¢¼Ó½Ãµµ°¡ ÀÖ¾ú½À´Ï´Ù.%s%s\r\n",
 ...
 /*   if (!*arg || strlen(arg) > MAX_PWD_LENGTH || strlen(arg) < 3 ||
	!str_cmp(arg, GET_PC_NAME(d->character))) {
      write_to_output(d, "\r\n»ç¿ëÇÒ ¼ö ¾ø´Â ÆÐ½º¿öµåÀÔ´Ï´Ù.\r\nÆÐ½º¿öµå:");
      return;
    } */
	
		if (!*arg) {
      write_to_output(d, "\r\n#1 »ç¿ëÇÒ ¼ö ¾ø´Â ÆÐ½º¿öµåÀÔ´Ï´Ù.\r\nÆÐ½º¿öµå:");
      return;
    }
	  if (strlen(arg) > MAX_PWD_LENGTH) {
      write_to_output(d, "\r\n#2 »ç¿ëÇÒ ¼ö ¾ø´Â ÆÐ½º¿öµåÀÔ´Ï´Ù.\r\nÆÐ½º¿öµå:");
      return;
    }
	  if (strlen(arg) < 3) {
      write_to_output(d, "\r\n#3 »ç¿ëÇÒ ¼ö ¾ø´Â ÆÐ½º¿öµåÀÔ´Ï´Ù.\r\nÆÐ½º¿öµå:");
      return;
    }
	  if (!str_cmp(arg, GET_PC_NAME(d->character))) {
      write_to_output(d, "\r\n#4 »ç¿ëÇÒ ¼ö ¾ø´Â ÆÐ½º¿öµåÀÔ´Ï´Ù.\r\nÆÐ½º¿öµå:");
      return;
    }
  ...
  case CON_CHPWD_VRFY:
    if (strncmp(CRYPT(arg, GET_PASSWD(d->character)), GET_PASSWD(d->character),
		MAX_PWD_LENGTH)) {
      write_to_output(d, "\r\nÆÐ½º¿öµå°¡ ÀÏÄ¡ÇÏÁö ¾Ê½À´Ï´Ù... ±âÁ¸ Á¤º¸¸¦ ÃÊ±âÈ­ÇÕ´Ï´Ù.\r\nÆÐ½º¿öµå:");
    ...
    if (STATE(d) == CON_CNFPASSWD) {
      write_to_output(d, "\r\nÄ³¸¯ÅÍÀÇ »ý¹°ÇÐÀû ¼ºº°Àº ¹«¾ùÀ¸·Î ÇÏ½Ã°Ú½À´Ï±î? (\t(³²\t)/\t(¿©\t))? ");
      STATE(d) = CON_QSEX;
    } else {
      save_char(d->character);
      write_to_output(d, "\r\n¿Ï·á.\r\n%s", CONFIG_MENU);
  ...
  case CON_QSEX:		/* query sex of new user         */
    if(!strcmp(arg, "³²") || !strcmp(arg, "³²¼º"))
      d->character->player.sex = SEX_MALE;
    else if(!strcmp(arg, "¿©") || !strcmp(arg, "¿©¼º"))
      d->character->player.sex = SEX_FEMALE;
    else {
      write_to_output(d, "ÁË¼ÛÇÕ´Ï´Ù, ÇöÀç Áö¿øµÇÁö ¾Ê´Â ¼ºº°ÀÔ´Ï´Ù..\r\n"
		"³²¼º ¶Ç´Â ¿©¼º Áß °¡±î¿î ¼º Á¤Ã¼¼ºÀº ¹«¾ùÀÔ´Ï±î?");
      return;
    }

    write_to_output(d, "È¸¿ø°¡ÀÔÀÌ ¿Ï·áµÇ¾ú½À´Ï´Ù!\r\nÁ÷¾÷ °ü·Ã ¾È³»»çÇ×\r\nÃÊ±â Á÷¾÷Àº ¹«Á÷ÀÔ´Ï´Ù. °ÔÀÓ ³»¿¡¼­ Á÷¾÷ Äù½ºÆ®¸¦ ÅëÇØ ¿øÇÏ´Â Á÷¾÷À¸·Î ÀüÁ÷ÀÌ °¡´ÉÇÕ´Ï´Ù.\r\n\r\n¿£ÅÍÅ°¸¦ ´­·¯ÁÖ¼¼¿ä\r\n"); // class_menu
    STATE(d) = CON_QCLASS;
    break;
...
//    } else
  ...
  case CON_RMOTD:		/* read CR after printing motd   */
    ...
    if (IS_HAPPYHOUR > 0){
      write_to_output(d, "\r\n");
      write_to_output(d, "\tyÇöÀç Çàº¹½Ã°£ ÀÌº¥Æ®°¡ ÁøÇàÁßÀÔ´Ï´Ù!\tn\r\n");
      write_to_output(d, "\r\n");
    }
  ...
  case CON_MENU: {		/* get selection from main menu  */
    ...
    case '1':
      ...
      act("$n´ÔÀÌ Á¢¼ÓÇÏ¼Ì½À´Ï´Ù.", TRUE, d->character, 0, 0, TO_ROOM);
	...
	send_to_char(d->character, "%s", CONFIG_START_MESSG);
      ...
      if (has_mail(GET_IDNUM(d->character)))
	send_to_char(d->character, "ÆíÁö°¡ µµÂøÇß½À´Ï´Ù.\r\n");
